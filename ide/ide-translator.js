...
function compile(source,program){
 ...
 const merge=(name,sentence,rows=false)=>{if(!p.current){warn(sentence,'there is no open file before the merge');return;}if(!p.inputs.includes(name))p.inputs.push(name);if(rows||p.kind==='table'){const next=temp('.csv');add(`csvstack "$CURRENT" ${q(name)} > "${next}"`);add(`CURRENT="${next}"`);p.kind='table';p.ext='.csv';p.tools.add('csvstack');}else if(['fasta','fastq'].includes(p.kind)&&['fasta','fastq'].includes(kind(name))){const next=temp();add(`cat "$CURRENT" ${q(name)} > "${next}"`);add(`CURRENT="${next}"`);}else warn(sentence,'the file types are not clearly compatible');};
 ...
 for(const item of raw){if(!item.text.endsWith('.'))throw new Error(`Line ${item.line} needs a period.`);const s=item.text.slice(0,-1).trim();let m;
  ...
  if((m=s.match(/^Open (?:the large file|the file) (.+)$/i))){open(m[1]);continue;}
  ...
  if((m=s.match(/^Merge (?:the result|it) with (.+)$/i))){merge(m[1],s);continue;}
