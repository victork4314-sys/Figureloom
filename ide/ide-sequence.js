(() => {
  ...
  const rules = [
    ...
    ['removeContaining', /^Remove sequences containing (.+)$/i], ['keepContaining', /^Keep only sequences containing (.+)$/i],
    ['use', /^Use the sequence named (.+)$/i], ['dnaRna', /^Convert the DNA to RNA$/i], ['rnaDna', /^Convert the RNA to DNA$/i],
    ['reverse', /^Find the reverse complement$/i], ['translate', /^Translate the DNA into protein$/i],
    ['showFirst', /^Show the first ([1-9][0-9]*) sequences?$/i], ['show', /^Show the (?:result|file)$/i],
    ['save', /^Save the result as (.+)$/i], ['say', /^Say (.+)$/i]
  ];
  const domain = new Set(['count','keepLong','removeShort','removeContaining','keepContaining','use','dnaRna','rnaDna','reverse','translate','showFirst']);
  ...
  function detect(items) {
    for (const item of items) {
      for (const [action, pattern] of rules) {
        const match = item.sentence.match(pattern);
        if (match && (domain.has(action) || (action === 'open' && isFasta(match[1])))) return true;
      }
    }
    return false;
  }
  ...
  function reverseComplement(sequence) {
    ...
    return Array.from(sequence).reverse().map((character) => {
      const upper = character.toUpperCase(); const replacement = map[upper] || upper;
      return character === character.toLowerCase() ? replacement.toLowerCase() : replacement;
    }).join('');
  }
    ...
    try {
      ...
      for (const instruction of instructions) {
        ...
        if (instruction.action === 'open') {
          const found = api.findFile(files,value);
          ...
          records = parseFasta(files[found],found);
          api.addSection('Opened the file',{ paragraphs:[found], bigValue:records.length.toLocaleString() });
          continue;
        }
        ...
        if (instruction.action === 'count') api.addSection('Sequences',{ bigValue:records.length.toLocaleString() });
          ...
          files[value] = encodeFasta(records); api.writeFiles(files); api.addSection('Saved the result',{ file:{ name:value, description:'Saved in Files' } });
        }
      }
      api.finishRun();
  ...
  const highlights = [
    [/^(Count the sequences)(\.)$/i,['c','p']], [/^(Keep only sequences longer than )([0-9]+)( bases?)(\.)$/i,['c','v','c','p']],
    [/^(Remove sequences shorter than )([0-9]+)( bases?)(\.)$/i,['c','v','c','p']], [/^(Remove sequences containing )(.+)(\.)$/i,['c','v','p']],
    [/^(Keep only sequences containing )(.+)(\.)$/i,['c','v','p']], [/^(Use the sequence named )(.+)(\.)$/i,['c','v','p']],
    [/^(Convert the (?:DNA to RNA|RNA to DNA))(\.)$/i,['c','p']], [/^(Find the reverse complement)(\.)$/i,['c','p']],
    [/^(Translate the DNA into protein)(\.)$/i,['c','p']], [/^(Show the first )([0-9]+)( sequences?)(\.)$/i,['c','v','c','p']]
  ];
