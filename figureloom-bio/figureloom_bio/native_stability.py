...
class StableExactWebSyntaxHighlighter(ExactWebSyntaxHighlighter):
    """The web-style highlighter, guarded against data and generated files."""

    def __init__(self, document, dark: bool = False) -> None:
        super().__init__(document, dark)
        # A file is loaded immediately after the editor is created. Starting off
        # prevents a previous CSV, FASTA, SVG, or result file from being parsed as
        # a program before the active filename is known.
        self.enabled = False

    def highlightBlock(self, text: str) -> None:  # noqa: N802 - Qt API name
        if not self.enabled:
            return
        if len(text) > MAX_HIGHLIGHT_LINE:
            # A valid FigureLoom Bio program should never need one 20,000-character
            # instruction. Skipping the line is safer than freezing the whole app.
            return
...
def install_native_stability(native_ide_module: Any) -> type[Any]:
    ...
    class StableNativeIdeWindow(base):
        def load_active_file(self) -> None:
            # This must happen before setPlainText. The previous implementation did
            # it afterwards, allowing a huge SVG line to enter the program parser.
...
def native_stability_self_test(native_ide_module: Any) -> dict[str, Any]:
    """Reproduce the real saved-on-a-large-SVG startup that previously crashed."""
...
def run_stable_ide(native_ide_module: Any, arguments: list[str] | None = None) -> int:
    try:
        ...
        from .platform_qt import crash_report, simple_explanation
