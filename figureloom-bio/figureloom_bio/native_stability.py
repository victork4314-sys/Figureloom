...
class StableExactWebSyntaxHighlighter(ExactWebSyntaxHighlighter):
    ...
    def __init__(self, document, dark: bool = False) -> None:
        ...
        # The active file is loaded immediately after the editor is created. Start
        # disabled so a saved CSV, FASTA, SVG, or result file is never parsed as a
        # program before the filename is known.
        self.enabled = False
...
def install_native_stability(native_ide_module: Any) -> type[Any]:
    ...
    class StableNativeIdeWindow(base):
        def load_active_file(self) -> None:
            # This must happen before setPlainText. Doing it afterwards lets a huge
            # generated SVG line enter the program parser during startup.
