        ...
        try:
            ...
            worker.failed.emit(
                simple_explanation(
                    ...
                    "Save the program, close and reopen FigureLoom Bio, then try again. If it repeats, run Repair and keep the crash report.",
...
def install_desktop_tool_reliability(platform_qt_module: Any) -> None:
            ...
            try:
                ...
                with urlopen(request, timeout=120) as response, destination.open("wb") as output:
                    ...
                    while True:
                        ...
                        received += len(block)
                        if total:
                            self.progress.emit(min(90, 8 + int(received / total * 82)), "Downloading the current installer")
    ...
    def manager_finished(window: Any, success: bool, message: str, path: str) -> None:
        ...
        # The OS installer is already open. Closing the updater releases its own
        # executable so Windows can replace it during the update.
    ...
    original_test_worker_run = platform_qt_module.TestWorker.run

    @Slot()
    def test_worker_run(worker: Any) -> None:
        try:
            original_test_worker_run(worker)
        except Exception as error:
            ...
            worker.finished.emit(
                ...
                simple_explanation(
                    ...
                    "Run Repair, then try the test again and keep the crash report if it repeats.",
                    technical_detail=f"{error}\n\nCrash report: {report}",
                ),
                str(report.parent),
            )

    platform_qt_module.TestWorker.run = test_worker_run
    ...
    def safe_show_manager() -> int:
        try:
            ...
            return _show_startup_crash(platform_qt_module, "Updater", error, "reinstall FigureLoom Bio from the official download")

    def safe_show_test() -> int:
        try:
            return original_show_test()
        except Exception as error:
            return _show_startup_crash(platform_qt_module, "Test-Tool", error, "run Repair from the updater")
