...
def _action_for(message: str) -> tuple[str, str]:
    ...
    return (
        "FigureLoom Bio reached a step it could not complete safely. It stopped instead of guessing or producing a misleading result.",
        "Read the original message below, correct that line or its input data, and run the program again.",
    )


def explain_plain_message(message: str) -> str:
    text = str(message).strip()
    if not text or ("What happened" in text and "How to fix it" in text):
        return text
