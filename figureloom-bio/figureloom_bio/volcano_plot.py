...
def install_volcano_plot(runner_class: type[Any]) -> None:
    ...
    def run_instruction(runner: Any, instruction: Any) -> None:
        if instruction.action != "volcano_plot":
            original_run_instruction(runner, instruction)
            return
        _create_volcano_plot(runner, instruction.values[0], instruction.values[1])
...
def _create_volcano_plot(runner: Any, effect_requested: str, p_requested: str) -> None:
    ...
    effect_column = _find_column(table, effect_requested)
    p_column = _find_column(table, p_requested)
    ...
    if not raw:
        raise FigureLoomBioError(
            ...
            f"{effect_column} must contain numeric effect sizes such as log2 fold change.\n"
...
def _volcano_svg(points: list[dict[str, Any]], effect_label: str, p_label: str) -> str:
    ...
    for threshold in (-EFFECT_THRESHOLD, EFFECT_THRESHOLD):
        px = x(threshold)
        parts.append(
    ...
    for point in sorted(points, key=lambda item: item["direction"] != "not-significant"):
        ...
        parts.append(
            ...
            f'fill="{colors[direction]}" fill-opacity="0.78" data-significance="{direction}">'
