...
def _create_volcano_plot(runner: Any, effect_requested: str, p_requested: str) -> None:
    ...
    lines = [
        "Points plotted", f"{len(points):,}", "", "Significantly higher", f"{higher:,}", "",
        "Significantly lower", f"{lower:,}", "", "Not significant", f"{neutral:,}", "",
        "Thresholds", f"Absolute effect at least {EFFECT_THRESHOLD:g}; p value at most {P_THRESHOLD:g}", "", "Saved", name,
    ]
    if invalid_effect or invalid_p:
        lines.extend(["", "Rows skipped", f"{invalid_effect + invalid_p:,}", "", "Why rows were skipped", f"Missing or non-numeric effect: {invalid_effect:,}; invalid p value: {invalid_p:,}"])
...
def _volcano_svg(points: list[dict[str, Any]], effect_label: str, p_label: str) -> str:
    ...
    for point in sorted(points, key=lambda item: item["direction"] != "not-significant"):
        ...
        parts.append(
            f'<circle cx="{x(float(point["effect"])):.2f}" cy="{y(float(point["score"])):.2f}" r="4.2" fill="{color}" fill-opacity="0.78" data-significance="{point["direction"]}">'
            f'<title>{escape(str(point["label"]))}: effect {float(point["effect"]):.4g}, p {float(point["p"]):.4g}</title></circle>'
        )
    labelled = sorted((point for point in points if point["direction"] != "not-significant"), key=lambda item: (float(item["score"]), abs(float(item["effect"]))), reverse=True)[:8]
