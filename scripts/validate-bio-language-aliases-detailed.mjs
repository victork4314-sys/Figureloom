...
const replacement = "if (typeof saved[name] !== 'string' || !saved[name].trimStart().startsWith('<svg')) { const value = saved[name]; const keys = Object.keys(saved).filter((key) => /(?:svg|scatter|histogram|bar|pca|volcano|heat|box)/i.test(key)); fail(`The browser did not save the real generated SVG ${name}. Type: ${typeof value}. Prefix: ${JSON.stringify(String(value ?? '').slice(0, 160))}. Figure-like keys: ${keys.join(', ') || '(none)'}.`); }";
