(() => {
  if (window.__figureLoomFinalSessionCoreV1) return;
  window.__figureLoomFinalSessionCoreV1 = true;
  ...
  style.id = 'figureloomFinalSessionCoreStyle';
  style.textContent = `
    /* Title and X are inside one project-tab box. */
    html[data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-wrap{
      position:relative!important;display:grid!important;grid-template-columns:minmax(0,1fr) 22px!important;
      align-items:center!important;column-gap:0!important;flex:0 1 190px!important;min-width:92px!important;
      max-width:190px!important;height:29px!important;min-height:29px!important;margin:0!important;padding:0!important;
      overflow:hidden!important;border:1px solid transparent!important;border-bottom:0!important;border-radius:9px 9px 0 0!important;
      background:transparent!important;color:var(--figureloom-ui-muted,#60706c)!important;box-shadow:none!important;box-sizing:border-box!important
    }
    html[data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-wrap:hover{
      background:var(--figureloom-ui-soft,#edf3f1)!important;color:var(--figureloom-ui-text,#172321)!important
    }
    html[data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-wrap.active{
      ...
      color:var(--figureloom-ui-text,#172321)!important;box-shadow:0 -3px 10px var(--figureloom-ui-shadow-soft,rgba(12,46,40,.08))!important
    }
    html[data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-wrap>.project-tab{
      ...
      width:100%!important;min-width:0!important;height:28px!important;min-height:28px!important;margin:0!important;
      padding:4px 6px 4px 9px!important;overflow:hidden!important;border:0!important;border-radius:0!important;
      background:transparent!important;color:inherit!important;box-shadow:none!important;box-sizing:border-box!important
    }
    ...
    html[data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-wrap>.project-tab-close{
      ...
      color:inherit!important;box-shadow:none!important;font-size:14px!important;line-height:1!important;opacity:.78!important
    }
    ...
    html[data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-scroll>.project-tab-add{
      align-self:end!important;flex:0 0 29px!important;display:grid!important;place-items:center!important;
      width:29px!important;min-width:29px!important;max-width:29px!important;height:29px!important;min-height:29px!important;
      max-height:29px!important;margin:0 0 0 2px!important;padding:0!important;border:1px solid var(--figureloom-ui-line,#cddbd7)!important;
      border-bottom:0!important;border-radius:9px 9px 0 0!important;background:var(--figureloom-ui-soft,#edf3f1)!important;
      color:var(--figureloom-ui-text,#172321)!important;box-shadow:none!important;font-size:16px!important;font-weight:700!important;line-height:1!important
    }

    /* Compact Recovery History. */
    html[data-figureloom-device-class="desktop"] body #historyDrawer{
      width:min(420px,calc(100vw - 48px))!important;max-width:min(420px,calc(100vw - 48px))!important;
      top:72px!important;right:16px!important;bottom:auto!important;max-height:calc(100vh - 96px)!important;
    ...
    /* MCP activity pointer. */
    .figureloom-mcp-agent-cursor{
      position:fixed;z-index:2147483600;left:0;top:0;display:flex;align-items:flex-start;gap:5px;pointer-events:none;
      opacity:0;transform:translate3d(-120px,-120px,0);transition:transform .25s cubic-bezier(.2,.8,.2,1),opacity .12s ease;
      filter:drop-shadow(0 5px 10px rgba(0,0,0,.16));will-change:transform,opacity
    }
    .figureloom-mcp-agent-cursor.visible{opacity:1}
    .figureloom-mcp-agent-cursor .mcp-pointer{position:relative;display:block;width:18px;height:24px;color:var(--mcp-agent-color,var(--figureloom-ui-accent,#2f7468))}
    .figureloom-mcp-agent-cursor .mcp-pointer::before{
    ...
    .figureloom-mcp-agent-cursor .mcp-agent-label{
      display:grid;gap:1px;max-width:190px;padding:5px 7px;border:1px solid var(--figureloom-ui-line,#cddbd7);
      border-radius:8px;background:var(--figureloom-ui-surface-glass,rgba(255,255,255,.96));color:var(--figureloom-ui-text,#172321);
      box-shadow:0 7px 18px var(--figureloom-ui-shadow-soft,rgba(12,46,40,.16));backdrop-filter:blur(8px);
      font:700 9px/1.15 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;white-space:nowrap
    }
    .figureloom-mcp-agent-cursor .mcp-agent-label small{max-width:175px;overflow:hidden;text-overflow:ellipsis;color:var(--figureloom-ui-muted,#60706c);font-size:7.5px;font-weight:600}
    html[data-figureloom-theme="dark"] .figureloom-mcp-agent-cursor .mcp-agent-label{background:rgba(36,44,42,.96);border-color:#50605c;color:#eef7f4}
    @media(max-width:760px){.figureloom-mcp-agent-cursor .mcp-agent-label small{display:none}}
    @media(prefers-reduced-motion:reduce){.figureloom-mcp-agent-cursor{transition:opacity .1s ease}}
  `;
  ...
  function enforceTabsAndRecovery() {
    ...
    scroll?.querySelectorAll('.project-tab-add-inline').forEach(node => { if (node !== add) node.remove(); });
  ...
  function observeRail() {
    const rail = document.getElementById('projectTabRail');
    if (!rail || rail === observedRail) return;
    railObserver?.disconnect();
    observedRail = rail;
    railObserver = new MutationObserver(scheduleRefresh);
    railObserver.observe(rail, { childList:true, subtree:true, attributes:true, attributeFilter:['class','aria-selected'] });
  }
  ...
  function targetForCommand(command, args = {}) {
    ...
    const selectors = [];
    if (/export|render|screenshot/.test(name)) selectors.push('#exportButton','.export-button','#canvas');
    ...
    if (objectId && window.CSS?.escape) selectors.unshift(`#canvas .canvas-object[data-id="${CSS.escape(objectId)}"]`);
  ...
  function createAgentCursor(sessionId) {
    ...
    cursor.innerHTML = '<span class="mcp-pointer mcp-paw" aria-hidden="true"></span><span class="mcp-agent-label"><b>MCP agent</b><small>Working</small></span>';
  ...
  function showAgentActivity(detail = {}) {
    ...
    state.element.querySelector('b').textContent = normalizedAgentName(detail.clientName);
    ...
    const point = targetPoint(targetForCommand(detail.command,detail.args || {}),sessionId);
    ...
    if (detail.phase === 'end') state.hideTimer = setTimeout(() => state.element.classList.remove('visible'), detail.ok === false ? 1800 : 1000);
  }
  ...
  addEventListener('scicanvas-cloud-opened', scheduleRefresh);
  addEventListener('scicanvas-cloud-saved', scheduleRefresh);
  addEventListener('scicanvas-cloud-disconnected', scheduleRefresh);
  addEventListener('figureloom-project-opened', scheduleRefresh);
  addEventListener('resize', scheduleRefresh, { passive:true });
  document.getElementById('documentName')?.addEventListener('input', scheduleRefresh);
  refresh();
  setTimeout(scheduleRefresh,250);
  setTimeout(scheduleRefresh,1000);

  window.FigureLoomFinalSessionPolish = Object.freeze({ refresh, showAgentActivity, enforceTabsAndRecovery });
})();
